    ...
    try:
        if message_text.startswith('delivered') or message_text.startswith('paid'):
            ...
            if update_data:
                ...
                await send_whatsapp_message(phone_number, reply_msg)
        else:
            await send_whatsapp_message(
                phone_number,
                "Send 'DELIVERED', 'PAID CASH', or 'PAID UPI' to update order status."
            )
...
async def handle_whatsapp_message(message_data: IncomingMessage):
    """Process incoming WhatsApp messages and generate responses (Legacy endpoint)"""
    try:
        phone_number = message_data.phone_number
        message_text = message_data.message.strip().lower()
        ...
        if delivery_person:
            await handle_delivery_boy_message(phone_number, message_text, delivery_person)
            return MessageResponse(reply="Message processed", success=True)
        else:
            await handle_customer_message(phone_number, message_text)
            return MessageResponse(reply="Message processed", success=True)
